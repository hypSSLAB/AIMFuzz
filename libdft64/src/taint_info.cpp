...
void insert_taint_mem_info(int ins_type, ADDRINT addr, char value, tag_t tag, string func_name) {
    ...
    if (monitor_taint_flag) {
        ...
        cout << "tag : " << dec << tag << ", val : " << hex << (tagVal[tag] & 0xff) << endl;
    }
...
void print_func_taint_mem_info() {
    ...
    cout << "===============taint tag================" << endl;
    prev_addr = 0;
    for (auto iter_map = taint_mem_list.begin(); iter_map != taint_mem_list.end(); iter_map++) {        
...
void get_score(FuncNode *my_node) {
    ...
    for (auto bbl_iter = my_node->taint_inst.begin(); bbl_iter != my_node->taint_inst.end(); bbl_iter++) {
        ...
        for (auto ins_iter = inner_map.begin(); ins_iter != inner_map.end(); ins_iter++) {
            my_score += (1 + smartln((double)(ins_iter->second))) * 100;
        }
...
void print_taint_info() {
    ...
    get_score(root_node);
    /*
    cout << endl << "===============taint func===============" << endl;
    cout << "total : " << dec << func_called.size() << endl;
    vector<pair<string, unsigned long>> v_tmp2(taint_func.begin(), taint_func.end());
    sort(v_tmp2.begin(), v_tmp2.end(), [] (const auto &x, const auto &y) {return x.second > y.second;});
    for (auto iter = v_tmp2.begin(); iter != v_tmp2.end(); iter++) {
        if (iter->second < 1) {
            continue;
        }

        if (taint_seq[iter->first] < read_seq_num) {
            continue;
        }
        ...
        new_file << iter->first << "," << iter->second << "," << taint_seq[iter->first] << "," << taint_depth[iter->first] << endl;
    }
    new_file.close();
    */
...
void print_tagged_memory() {
	...
	while(fgets(map_buf, 1024, pFile)) {
		...
		if (1) {
            ...
            while (cur_addr < max) {
                tag_t cur_tag;
                if ((cur_tag = tagmap_getb(cur_addr)) != 0) {
                    cout << "[" << hex << cur_addr << "] : ";
                    cout << "{" << dec << cur_tag << ", " << hex << ((uint32_t)(tagVal[cur_tag]) & 0xff) << "}" << endl;
                }
                cur_addr++;
