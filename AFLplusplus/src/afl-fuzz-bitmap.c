 ...
 * return has_new_bits(). */
GArray *global_edge_list = NULL;
GArray *edge_list = NULL;

extern int path_coverage_mode;
extern void *edge_address;

inline u8 has_new_bits_unclassified(afl_state_t *afl, u8 *virgin_map) {

  /* Handle the hot path first: no new coverage */

  if (path_coverage_mode) {
    uint64_t *single_edge_list = edge_address;
    int edge_size = 0;
    for (int i = 0; i < 16384; i++) {
      uint64_t cur_loc = single_edge_list[i];
      if (cur_loc == (uint64_t)-1) {
        break;
      }
      edge_size++;
    }
    // fprintf(stderr, "edge size : %d\n", edge_size);
    for (int i = 0; i < (int)global_edge_list->len; i++) {
      GArray *target_list = g_array_index(global_edge_list, GArray*, i);
      if (edge_size != (int)target_list->len) {
        // new path coverage
        continue;
      }
      for (int j = (int)target_list->len-1; j >= 0; j--) {
        if (g_array_index(target_list, uint64_t, j) != single_edge_list[j]) {
          break;
        }
        if (j == 0) {
          // fprintf(stderr, "no path coverage found\n");
          return 0;
        }          
      }
    }   
    edge_list = g_array_new(FALSE, FALSE, sizeof(uint64_t));
    g_array_append_vals(edge_list, single_edge_list, edge_size);
    g_array_append_val(global_edge_list, edge_list);    

    // fprintf(stderr, "new coverage found, total : %d\n", global_edge_list->len);
    return 2;
  }
...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  // fprintf(stdout, "save if interesting\n");

  if (unlikely(len == 0)) { fprintf(stderr, "len = 0\n"); return 0; }
