...
/* Main entry point */
extern GArray *global_edge_list;
int path_shmid;
void *edge_address;
int path_coverage_mode;
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  // shared memory for path coverage
  if ((path_shmid = shmget(11223344, 8*32768, IPC_CREAT|0666)) == -1) {
    fprintf(stderr, "afl shmget failed : %d\n", errno );
    return -1;
  }
  if ((edge_address=shmat(path_shmid, NULL, 0)) == (void *)-1) {
    fprintf(stderr, "afl shmat failed\n");
    return -1;
  }
  global_edge_list = g_array_new(FALSE, FALSE, sizeof(GArray *));
  path_coverage_mode = !!getenv("PATH_COVERAGE");
  if (path_coverage_mode) {
    OKF("enable path coverage\n");
  } else {
    OKF("use edge coverage\n");
  }
