...
extern "C" size_t input_size;
extern "C" int *taint_index;
MutationDispatcher::MutationDispatcher(Random &              Rand,
  ...
  DefaultMutators.insert(
      DefaultMutators.begin(),
      {
          ...
          // "InsertRepeatedBytes"},
          {&MutationDispatcher::Mutate_ChangeByte, "ChangeByte"},               // done
          {&MutationDispatcher::Mutate_ChangeBit, "ChangeBit"},                 // done
          {&MutationDispatcher::Mutate_ShuffleBytes, "ShuffleBytes"},           // done
          {&MutationDispatcher::Mutate_ChangeASCIIInteger, "ChangeASCIIInt"},   // done
          {&MutationDispatcher::Mutate_ChangeBinaryInteger, "ChangeBinInt"},    // done
          {&MutationDispatcher::Mutate_CopyPart, "CopyPart"},                   // done
...
size_t MutationDispatcher::Mutate_ShuffleBytes(uint8_t *Data, size_t Size,
  ...
  */
  int shuffle_success = 0;
  size_t ShuffleAmount = Rand(8);
  size_t currentAmount = 0;
  for (int i = 0; i < input_size; i++) {
    int j;
    if (currentAmount >= ShuffleAmount) {
      break;
    }
    currentAmount++;
    ...
    if (taint_index[i] || taint_index[j]) {
      // swap
      uint8_t tmp = Data[i];
      Data[i] = Data[j];
      Data[j] = tmp;
...
size_t MutationDispatcher::CopyPartOf(const uint8_t *From, size_t FromSize,
  ...
  while (!is_taint) {
    ...
    ToBeg = Rand(ToSize);
    CopySize = Rand(ToSize - ToBeg) + 1;
    assert(ToBeg + CopySize <= ToSize);
    CopySize = std::min(CopySize, FromSize);
    FromBeg = Rand(FromSize - CopySize + 1);
    assert(FromBeg + CopySize <= FromSize);

    for (int i = 0; i < CopySize; i++) {
      if ((ToBeg+i) < input_size && taint_index[ToBeg+i]) {
        is_taint = 1;
        break;
      }
    }
...
size_t MutationDispatcher::InsertPartOf(const uint8_t *From, size_t FromSize,
  ...
  size_t AvailableSpace, MaxCopySize, CopySize, FromBeg, ToInsertPos, TailSize;
  int is_taint = 0;
  int rand_try = 0;
  ...
  while (!is_taint) {
    ...
    MaxCopySize = std::min(AvailableSpace, FromSize);
    CopySize = Rand(MaxCopySize) + 1;
    FromBeg = Rand(FromSize - CopySize + 1);
    assert(FromBeg + CopySize <= FromSize);
    ToInsertPos = Rand(ToSize + 1);
    assert(ToInsertPos + CopySize <= MaxToSize);
    TailSize = ToSize - ToInsertPos;

    for (int i = 0; i < TailSize; i++) {
      if ((ToInsertPos+CopySize+i) < input_size && taint_index[ToInsertPos+CopySize+i]) {
        is_taint = 1;
        break;
      }
    }
    ...
    for (int i = 0; i < CopySize; i++) {
      if ((ToInsertPos+i) < input_size && taint_index[ToInsertPos+i]) {
        is_taint = 1;
        break;
      }
    }
...
size_t ChangeBinaryInteger(uint8_t *Data, size_t Size, Random &Rand) {
  ...
  while (!is_taint) {
    ...
    rand_try++;
    Off = Rand(Size - sizeof(T) + 1);

    for (int i = 0; i < sizeof(T); i++) {
      if ((Off+i) < input_size && taint_index[Off+i]) {
        is_taint = 1;
        break;
      }
    }
