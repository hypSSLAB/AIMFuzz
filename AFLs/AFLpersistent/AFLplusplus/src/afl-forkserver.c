...
afl_fsrv_write_to_testcase(afl_forkserver_t *fsrv, u8 *buf, size_t len) {
  ...
  if (likely(fsrv->use_shmem_fuzz)) {
    ...
    /* original qemu persistent mode does not create .cur_input when shared memory mode.
       since we need a memory map of initial input, create .cur_input to  
    */
    ...
    if (fd >= 0) {
      ...
      close(fd);
      src_path = getenv("IN_MEMORY_FUZZING_SOURCE_PATH");
      if (!src_path) {
        PFATAL("Unable to load IN_MEMORY_FUZZING_SOURCE_PATH");
      }
      
      sprintf(command, "cp %s %s", src_path, fsrv->out_file);
